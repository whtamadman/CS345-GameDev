...
public class DrPepperBoss : MonoBehaviour
{
    [Header("Boss Health")]
    ...
    [SerializeField] private GameObject[] burstfireProjectilePrefabs; // Array of possible projectile prefabs
    [Tooltip("Percentage chance for each projectile prefab (should sum to 1.0 or 100)")]
    [SerializeField] private float[] burstfireProjectileChances; // Array of chances for each prefab
    ...
    // List to keep track of all burstfire projectiles spawned by this boss
    private List<GameObject> burstfireProjectiles = new List<GameObject>();
   ...
   [SerializeField] private float spawnDelay = 2.0f;
    ...
    // Events
    public System.Action<DrPepperBoss> OnBossDeath;
    ...
    void Start()
    {
        ...
        Debug.Log($"Prefab assignments - Burstfire: {(burstfireProjectilePrefabs != null && burstfireProjectilePrefabs.Length > 0 ? "OK" : "MISSING")}, Phase1 Minions: {(phase1MinionPrefabs?.Length > 0 ? "OK" : "MISSING")}, Phase2 Minions: {(phase2MinionPrefabs?.Length > 0 ? "OK" : "MISSING")}, Phase3 Minions: {(phase3MinionPrefabs?.Length > 0 ? "OK" : "MISSING")}");
    ...
    private IEnumerator ChooseAndExecuteAttack()
    {
        ...
        if (enableMinionSpawnAttack && Time.time - lastMinionSpawnTime >= minionSpawnCooldown)
        {
            selectedAttack = 1;
        }
    ...
    private IEnumerator BurstfireProjectileAttack()
    {
        ...
        GameObject player = GameObject.FindWithTag(opponentTag);
        if (player == null || burstfireProjectilePrefabs == null || burstfireProjectilePrefabs.Length == 0) yield break;
        ...
        for (int i = 0; i < projectileCount; i++)
        {
            ...
            // Select a projectile prefab based on weighted chance
            GameObject selectedPrefab = burstfireProjectilePrefabs[0];
            if (burstfireProjectileChances != null && burstfireProjectileChances.Length == burstfireProjectilePrefabs.Length)
            {
                float total = burstfireProjectileChances.Sum();
                float rand = Random.Range(0f, total);
                float accum = 0f;
                for (int j = 0; j < burstfireProjectileChances.Length; j++)
                {
                    accum += burstfireProjectileChances[j];
                    if (rand <= accum)
                    {
                        selectedPrefab = burstfireProjectilePrefabs[j];
                        break;
                    }
                }
            }
            ...
            {
                // Fallback to uniform random if chances not set up
                selectedPrefab = burstfireProjectilePrefabs[Random.Range(0, burstfireProjectilePrefabs.Length)];
            }

            Debug.Log($"Creating burstfire projectile {i+1} using prefab {selectedPrefab.name} at {spawnPos}");

            GameObject burstfireProjectile = Instantiate(selectedPrefab, spawnPos, Quaternion.identity);
...
/// <summary>
/// Independent homing projectile behavior for DrPepperBoss
/// </summary>
public class DrPepperHomingProjectileBehavior : MonoBehaviour
{
...
/// <summary>
/// Simple projectile destroyer for projectiles without Projectile component (DrPepperBoss)
/// </summary>
public class DrPepperSimpleProjectileDestroy : MonoBehaviour
{
...
/// <summary>
/// Enforces constant movement for circle projectiles (DrPepperBoss)
/// </summary>
public class DrPepperCircleProjectileMovement : MonoBehaviour
{
